...
public class ChangeSample : MonoBehaviour {
	...
	private void OnFinishedImagePicker (string message) {
		if (LoadTextureFromImagePicker.IsLoaded()) {
			int width = obj.renderer.material.mainTexture.width;
			int height = obj.renderer.material.mainTexture.height;
			...
			if (texture) {
				// Load Texture
#if false
				Texture lastTexture = obj.renderer.material.mainTexture;
